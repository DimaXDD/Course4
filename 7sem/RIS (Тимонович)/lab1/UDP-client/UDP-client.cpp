...
int main(int argc, char* argv[])
{
	...
	try
	{
		...
		for (int i = 0; i < 10; i++)
		{
			...
			times[i] = setsincro.curvalue - end + start - _time;
			...
			std::cout << tm.wDay << "." << tm.wMonth << "." << tm.wYear << " - " << tm.wHour + 3 << ":" << tm.wMinute << ":" << tm.wSecond << ":"
		...
		std::cout << "Average correction: " << avgcorr / 10 << endl;

		long max = INT_MIN, min = INT_MAX, sum = 0;

		for (auto i = 0; i < 10; i++)
		{
			std::cout << "\nCorrection " << i + 1 << " = " << (times[i]);
