...
int main()
{
	...
	try
	{
		...
		while (count != 21)
		{
			...
			setsincro.correction = clock(); // correction;
			c = clock();
			/*cout << c;*/

			/*auto now = steady_clock::now();
			auto correction = duration_cast<milliseconds>(now.time_since_epoch()).count() - getsincro.correction;*/

			/*correction = c - getsincro.correction;
			
			averageCorrection[count - 1] = correction;
		...
		for (auto i = 0; i < 10; i++)
		{
			if (min > averageCorrection[i])
				min = averageCorrection[i];
			if (max < averageCorrection[i])
				max = averageCorrection[i];
		}

		cout << 
			"\nMax correction = " << max << endl <<
			"Min correction = " << min << endl;
